...
Page({
  ...
  async loadEventDetail() {
    console.log('=== loadEventDetail 开始加载 ===');
    this.setData({ loading: true });

    try {
      console.log('🔄 尝试调用真实API获取活动详情...');
      // 调用真实的API获取活动详情
      const app = getApp();
      ...
      if (response.success) {
        console.log('✅ API调用成功，使用真实数据');
        const eventData = response.data;
        console.log('📊 真实活动数据:', eventData);
        console.log('📊 can_enroll值:', eventData.can_enroll);
  ...
  loadMockEventDetail() {
    ...
    try {
      const mockEvent = {
        ...
        status_text: '进行中',
        status_icon: '📖',
        date_range: '2025-01-15 至 2025-02-14',
        ...
        max_participants: 20,
        enrollment_fee: 0,
        can_enroll: true, // 允许报名
        completed_today: 8,
        user_enrollment: null // 模拟无报名信息
      };

      console.log('🎭 模拟活动数据创建完成:', mockEvent);
      console.log('🎭 模拟数据 can_enroll 值:', mockEvent.can_enroll);

      // 模拟用户角色判断
      const userRole = this.determineUserRole(mockEvent);
      console.log('🎭 模拟数据用户角色:', userRole);
  ...
  handleFloatingAction() {
    console.log('=== handleFloatingAction 调试信息 ===');
    console.log('当前用户角色:', this.data.userRole);
    console.log('用户信息:', this.data.userInfo);
    ...
    if (this.data.userRole === 'participant') {
      // 参与者：直接跳转到打卡页面
      this.startCheckIn();
    } else if (this.data.userRole === 'observer') {
      // 围观者：显示报名规则确认弹窗
      this.showRuleConfirmModal();
    } else {
      // 游客：直接显示报名规则确认弹窗（因为用户已登录）
      this.showRuleConfirmModal();
    }
  },

  // 显示报名规则确认弹窗
  showRuleConfirmModal() {
    this.setData({
      showRuleConfirmModal: true
    });
  },

  // 隐藏报名规则确认弹窗
  hideRuleConfirmModal() {
    this.setData({
      showRuleConfirmModal: false
    });
  },

  // 确认参加活动
  async confirmJoinActivity() {
    console.log('=== 确认参加活动调试信息 ===');
    console.log('当前用户信息:', this.data.userInfo);
    console.log('当前活动信息:', this.data.eventInfo);
    console.log('can_enroll 值:', this.data.eventInfo?.can_enroll);
    console.log('current_participants:', this.data.eventInfo?.current_participants);
    console.log('max_participants:', this.data.eventInfo?.max_participants);
    console.log('enrollment_deadline:', this.data.eventInfo?.enrollment_deadline);
    ...
    // 检查活动是否可以报名
    if (!this.data.eventInfo?.can_enroll) {
      console.log('❌ 活动不允许报名 - can_enroll 为 false');
      wx.showToast({
        title: '活动已满员或已截止',
        icon: 'none'
      });
    ...
    // 检查活动人数是否已满
    if (this.data.eventInfo?.current_participants >= this.data.eventInfo?.max_participants) {
      console.log('❌ 活动人数已满');
      wx.showToast({
        title: '活动人数已满',
        icon: 'none'
    ...
    // 检查报名是否已截止
    if (this.data.eventInfo?.enrollment_deadline) {
      const deadline = new Date(this.data.eventInfo.enrollment_deadline);
      const now = new Date();
      if (now > deadline) {
        console.log('❌ 报名已截止');
        wx.showToast({
