...
class ReadingEventTestFixed < ActiveSupport::TestCase
  def setup
  ...
  test "can_start? should check conditions" do
    ...
    # 设置为报名状态且人数足够
    @reading_event.update!(status: :enrolling, start_date: Date.current, min_participants: 1)
    EventEnrollment.create!(user: @user, reading_event: @reading_event, status: 'enrolled', enrollment_date: Time.current)
  ...
  test "user_enrollment should return user enrollment" do
    ...
    # 已报名用户返回enrollment
    enrollment = EventEnrollment.create!(user: @user, reading_event: @reading_event, status: 'enrolled', enrollment_date: Time.current)
  ...
  test "should handle complete event lifecycle" do
    ...
    # 用户报名
    EventEnrollment.create!(user: @user, reading_event: event, status: 'enrolled', enrollment_date: Time.current)
    EventEnrollment.create!(user: @admin, reading_event: event, status: 'enrolled', enrollment_date: Time.current)
